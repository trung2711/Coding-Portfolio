...
int main()
{
    ...
    ftruncate(fd, sizeof(fileDrive)+sizeof(entry));
    fileDrive *D = mmap(0, sizeof(fileDrive), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);
    initializeDrive(D);

    printf("Testing directory creation\n");
    createDir("/testingDir", D);

    printf("\nTesting subdirectory creation\n");
    createDir("/testingDir/testingSubDir", D);

    printf("\nTesting file creation\n");
    createF("/testingDir/testingSubDir/myFile.txt", 0, D);

    printf("\nTesting file opening\n");
    fP * ptr = openF("/testingDir/testingSubDir/myFile.txt", D);
    printf("\nIf writeFile (below) is successful, then openFile also succeeded\n");
    ...
    printf("\nTesting file write\n");
    for(int i = 0; i<5; i++){
        writeF(ptr, "I am testing writeFile 5 times", 31, D);
    }
    ...
    int i = 0;
    int k;
    while((k = readF(buff, ptr, 31, D)) != 0&&i<5){
    ...
    deleteF("/testingDir/testingSubDir/myFile.txt", D);
    ...
    ptr = openF("/testingDir/testingSubDir/myFile.txt",D);
...
entry * getEntry(char *path, fileDrive *D){
    if (strcmp(path, "") == 0) {
        // return entry to root directory
        return &ROOT;
    }
    ...
    entry *meta = &NULLENTRY;
    // Initialize loop variables
    int found = 0; //1 if found the entry with the path
    int last = 1; //type of the last file entry read
    ...
    for (token = strtok(cpyPath, "/"); token != NULL; token = strtok(NULL, "/")) {
        ...
        for (int b = meta->startBlock; b != -1 && !found; b = D->FAT[b]) {
            for (int i = 0; i < BSIZE / sizeof(entry) && !found; i++) {
...
int createF(char *path, int type, fileDrive *D) {
    // Copy the path into another string
    char cpyPath[strlen(path)];
    strcpy(cpyPath, path);
    // Check if file already exists
    if (getEntry(cpyPath, D) != NULL) {
    ...
    int dir;
    entry *dirMeta = getEntry(cpyPath, D);
    ...
    for (; dir != -1; dir = D->FAT[dir]) {
		...
		getTime(D->data[dir].dir[i].timeCreated);
		getDate(D->data[dir].dir[i].dateCreated);
		//When file is first created, creation date(time) and modified date (time) are the same
		strcpy(D->data[dir].dir[i].timeModified, D->data[dir].dir[i].timeCreated);
		strcpy(D->data[dir].dir[i].dateModified, D->data[dir].dir[i].dateCreated);
		if(type==0){
    ...
    getTime(D->data[newBlock].dir[0].timeCreated);
    getDate(D->data[newBlock].dir[0].dateCreated);
    //When file is first created, creation date(time) and modified date (time) are the same
    strcpy(D->data[newBlock].dir[0].timeModified, D->data[newBlock].dir[0].timeCreated);
    strcpy(D->data[newBlock].dir[0].dateModified, D->data[newBlock].dir[0].dateCreated);
    if(type==0){
	printf("Successfully create file at path: %s\n", path);
    } else {
	printf("Successfully create directory at path %s\n", path);
...
int createDir(char * path, fileDrive * D){
	return createF(path, 1, D);
}

// Write to file pointer
int writeF(fP *file, char *data, int len, fileDrive *D) {
    // Write file will trigger a modification date/time change
    getTime(file->meta->timeModified);
    getDate(file->meta->dateModified);
    ...
    while (len > 0) {
	    ...
	    if(newB == -1){
 		printf("writeFile stopped because drive is full\n");
		return offset;
	    }
...
// Open file
fP *openF(char *path, fileDrive *D) {
    char cpyPath[strlen(path)];
    strcpy(cpyPath, path);
    // Create pointer for file entry
    entry *meta = getEntry(cpyPath, D);
...
int readF(char *buff, fP *file, int len, fileDrive *D) {
    // To advance pointer
    int offset = 0;
...
// Delete file
int deleteF(char *path, fileDrive *D) {
    char cpyPath[strlen(path)];
    strcpy(cpyPath, path);
    // Get the file's metadata
    entry *file = getEntry(cpyPath, D);
...
//print entry using entry struct
void printEntryInfo(entry * e){
