...
//myshell is a simple shell so it only works with command lines of 256 characters or less.
...
int executeNorm(char **command, int builtinNum);
int executeBg(char **command, int builtinNum);
int executeRedir(char **command, char *inputName, char *outputName, int isAppend, int builtinNum);
int executePipe(char ** command1, char ** command2, int builtinNum1, int builtinNum2);
...
//To call readme file from anywhere
...
int main(int args, char* argv[]){
    ...
    if(argv[1] != NULL){    //if reading from a file
        ...
        if((fp = fopen(argv[1], "r")) == NULL){
            fprintf(stderr,"Cannot open batchfile with name %s.\n",argv[1]);
            return 0;
        }
...
void process(char *line){
    ...
    //myshell is a simple shell. It executes 1 command/program at a time, or 2 commands/programs when invoking a pipe.
    ...
    for (; args[i]!=NULL; i++){
        ...
        if (strcmp(args[i], ">")==0){
            if(hasOutput){
                printf("%s\n","myshell only supports one output file at a time.");
                return;
            }
            hasOutput = 1;
            command1[i] = NULL;
            ...
            if(hasInput){
                printf("%s\n","myshell only supports one input file at a time.");
                return;
            }
            hasInput = 1;
            command1[i] = NULL;
    ...
    int builtinNum1 = checkBuiltins(command1[0]);
    //Use builtinNums to decide which functions to call (or program execution) later on
    if(isPipe){
        if(command2[0]==NULL){
            fprintf(stderr,"%s\n", "Cannot execute pipe: missing or ambiguous command 2");
        } else {
            int builtinNum2 = checkBuiltins(command2[0]);
            executePipe(command1,command2, builtinNum1, builtinNum2);
        }
        return;

    } else if(isBg){

        executeBg(command1, builtinNum1);
        return;

    } else if(hasInput|hasOutput){

        executeRedir(command1, inputName, outputName, isAppend, builtinNum1);
        return;

    } else {
        executeNorm(command1, builtinNum1);
    }
...
int executeNorm(char **command, int builtinNum){
    if(builtinNum!=-1){
        if((*builtins[builtinNum])(command)!=0){
            fprintf(stderr,"%s %s\n","Failed to execute built-in command:", builtin_cmd[builtinNum]);
            return -1;
        }
        return 0;
    }
    ...
    if ((pid=fork())==-1) {
        ...
        if (execvp(command[0], command) != 0) {
			fprintf(stderr,"Cannot start the program with the given arguments.\n");
		}
		exit(0);
    } else {
		wait(NULL);
        ...
        if(outputName[0]!='\0'){
            if (isAppend){
                if((fdOut = open(outputName, O_WRONLY|O_CREAT|O_APPEND,S_IRWXU|S_IRWXG|S_IRWXO))==-1){
                    fprintf(stderr,"%s\n","Cannot create output file. Please check for invalid file name and try again.");
                    exit(1);
                }
            } else {
...
int executePipe(char ** command1, char ** command2, int builtinNum1, int builtinNum2){
    ...
    if ((pid=fork())==0) {
        ...
        if(builtinNum1!=-1){
            if((*builtins[builtinNum1])(command1)!=0){
                fprintf(stderr,"%s %s\n","Failed to execute built-in command:", builtin_cmd[builtinNum1]);
                return -1;
            }
            exit(0);
        }
        ...
        if ((pid=fork())==0) {
            ...
            if(builtinNum2!=-1){
                if((*builtins[builtinNum2])(command2)!=0){
                    fprintf(stderr,"%s %s\n","Failed to execute built-in command:", builtin_cmd[builtinNum2]);
                    return -1;
                }
                exit(0);
            }
            if (execvp(command2[0], command2) != 0) {
                fprintf(stderr,"Cannot start the second program with the given arguments.\n");
                exit(1);
            }
        }
        close(fd[0]);
...
int checkBuiltins(char *args){
    ...
    int i = 0;
    for(;i<7;i++) {//8 is the number of built-in commands
